...
class SealExtractor:
    ...
    def process_image(self, image_path, page_num, min_radius=0):
        ...
        :param page_num: 页码
        :param min_radius: 最小半径过滤阈值（像素），小于此值的公章不保存
        """
        ...
        for i, seal_info in enumerate(seals):
            ...
            if seal_info['radius'] < min_radius:
                filtered_count += 1
                print(f"  公章 #{i + 1}: 半径 {seal_info['radius']}px < {min_radius}px, 已过滤")
                continue
        ...
        if filtered_count > 0:
            print(f"  (共过滤 {filtered_count} 个小尺寸公章)")
    ...
    def process_pdf(self, pdf_path, zoom=2.0, analyze_overlaps=True, overlap_threshold=0.1, min_radius=0):
        ...
        print("=" * 60)
        if min_radius > 0:
            print(f"半径过滤: 仅保存半径 >= {min_radius}px 的公章")
            print("=" * 60)
